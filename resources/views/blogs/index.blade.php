<x-app-layout>
    <div x-data="{ someValue: 'This value is outside the modal' }">
        <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
            <form method="POST" action="{{route('blogs.store')}}">
                ...
                <x-primary-button class="mt-4">{{ __('Valid') }}</x-primary-button>
            </form>
            <div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
                ...
                <div class="p-6 flex space-x-2">
                    ...
                    <div class="flex-1">
                            ...
                            <x-dropdown>
                                ...
                                <x-slot name="content">
                                    ...
                                    <form method="POST" action="{{ route('blogs.destroy', $blog) }}">
                                        @csrf
                                        @method('delete')
                                        <x-dropdown-link x-on:click.prevent="$dispatch('open-modal', 'confirm-blog-deletion',someValue = {{$blog}})">
                                            {{ __('Delete') }}
                                        </x-dropdown-link>
                                    </form>
                        ...
                        <p class="mt-4 text-lg text-gray-900">{{ $blog->message }} - {{ $blog->id }}</p>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <x-modal name="confirm-blog-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
            <form method="post" x-bind:action="`{{ route('blogs.destroy', '') }}/${someValue.id}`" class="p-6">
                @csrf
                @method('delete')
